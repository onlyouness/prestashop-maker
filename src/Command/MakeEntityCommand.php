...
class MakeEntityCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        while (true) {
            $fieldName = $io->ask('Field name (e.g. title) <fg=yellow> (press enter to stop)</>');
            if (! $fieldName) {
                break;
            }
        ...
        $this->generatorManager->process('entity', $module, [
            ...
            'table_name' => strtolower($entityName),
        ]);
        $io->success([
            "Entity for module $module created successfully!",
            "Location: modules/$module/src/Entity/$entityName.php",
        ]);
